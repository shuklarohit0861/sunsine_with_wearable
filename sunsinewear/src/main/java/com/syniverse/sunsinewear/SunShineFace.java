...
package com.syniverse.sunsinewear;
...
public class SunShineFace extends CanvasWatchFaceService {
    ...
    private class Engine extends CanvasWatchFaceService.Engine implements DataApi.DataListener,GoogleApiClient.ConnectionCallbacks,GoogleApiClient.OnConnectionFailedListener {
        ...
        public void onApplyWindowInsets(WindowInsets insets) {
            ...
            float dataSize = resources.getDimension(isRound
                    ? R.dimen.digital_data_size_round : R.dimen.digital_date_size);

            mDatePaint.setTextSize(dataSize);
        ...
        public void onAmbientModeChanged(boolean inAmbientMode) {
            ...
            if (mAmbient != inAmbientMode) {
                ...
                if (mLowBitAmbient) {
                    ...
                    mTemp.setAntiAlias(!inAmbientMode);
                }
                invalidate();
            }
        ...
        public void onDraw(Canvas canvas, Rect bounds) {
            ...
            String temp = highTemp +  "\u00b0"+ "  " + lowTemp+"\u00b0";

            String date = new SimpleDateFormat("EEE, d MMM yyyy", Locale.US).format(Calendar.getInstance().getTime());
           ...
           // mTextPaint.setTextSize(30);
            canvas.drawText(date,bounds.centerX()-(mDatePaint.measureText(date))/2,mYOffset+30,mDatePaint);
            canvas.drawLine(40,mYOffset +50,bounds.width()-40,mYOffset+50,line);
            if(highTemp != null) {
                canvas.drawText(temp, bounds.centerX() - (mTemp.measureText(temp)) / 2, mYOffset + 100, mTemp);
            }
        }
        ...
        public void updateWeather(DataMap config)
        {
            ...
            for(String configKey : config.keySet())
            {
                ...
                if (configKey.equals("High"))
                {
                 highTemp = config.getString(configKey);
                    uiUpdate = true;
                }
                else if(configKey.equals("Low"))
                {
                    lowTemp = config.getString(configKey);
                    uiUpdate = true;
                }
            }
            if(uiUpdate)
